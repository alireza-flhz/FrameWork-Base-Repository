...
public class EndToEndCrudTests : IClassFixture<SampleWebApplicationFactory>
{
    ...
    {
        var createResponse = await _client.PostAsJsonAsync("/api/v1/samples", new { Name = "first" });
        ...
        var getResponse = await _client.GetAsync($"/api/v1/samples/{created.Id}");
        Assert.Equal(HttpStatusCode.OK, getResponse.StatusCode);
        var fetched = await getResponse.Content.ReadFromJsonAsync<SampleDto>();
        Assert.Equal("first", fetched!.Name);

        var listResponse = await _client.GetAsync("/api/v1/samples?pageIndex=1&pageSize=10");
        Assert.Equal(HttpStatusCode.OK, listResponse.StatusCode);
        var page = await listResponse.Content.ReadFromJsonAsync<PagedResult<SampleDto>>();
        Assert.NotNull(page);
        Assert.True(page!.TotalCount >= 1);

        var updateResponse = await _client.PutAsJsonAsync($"/api/v1/samples/{created.Id}", new { Name = "changed" });
        Assert.Equal(HttpStatusCode.OK, updateResponse.StatusCode);
        var updated = await updateResponse.Content.ReadFromJsonAsync<SampleDto>();
        Assert.Equal("changed", updated!.Name);

        var deleteResponse = await _client.DeleteAsync($"/api/v1/samples/{created.Id}");
        Assert.Equal(HttpStatusCode.NoContent, deleteResponse.StatusCode);

        var afterDeleteResponse = await _client.GetAsync($"/api/v1/samples/{created.Id}");
    ...
    {
        var response = await _client.PostAsJsonAsync("/api/v1/samples", new { Name = "" });
    ...
    {
        var response = await _client.GetAsync("/api/v1/samples/999999");
